...
const generateChartData = (earthquakes: Earthquake[]) => {
  if (earthquakes.length < 5) return [
  ...
  return earthquakes.slice(0, 10).reverse().map(eq => ({
    name: new Date(eq.time).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' }),
...
function getStatusWilayah(earthquakes: Earthquake[]): { status: string; color: string } {
  if (earthquakes.length === 0) return { status: "Aman", color: "text-green-600" };
  const latest = earthquakes[0];
  if (!latest || !latest.magnitude) return { status: "Aman", color: "text-green-600" };
  if (latest.magnitude >= 6) return { status: "Bahaya", color: "text-red-600" };
  if (latest.magnitude >= 5) return { status: "Waspada", color: "text-orange-600" };
...
export default function Dashboard() {
  ...
  const [nearestShelters, setNearestShelters] = useState<(Shelter & { distance: number })[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      const [
        ...
        earthquakeApi.getAll({ limit: 100 }).catch(() => ({ data: [], total: 0, page: 1, limit: 100 })),
        publicFacilityApi.getAll().catch(() => []),
        roadApi.getRoadNetwork({ minLat: -8.05, maxLat: -7.75, minLon: 110.15, maxLon: 110.55 }).catch(() => null),
      ]);
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (!isWithinBantul(latitude, longitude)) {
          toast.error("Maaf, lokasi GPS Anda di luar batas wilayah Kabupaten Bantul.");
  ...
  const handleToggleRoutingMode = () => {
    ...
    if (!routingMode) toast.info("Mode Routing: Klik peta untuk pilih titik awal dan tujuan");
  };
  ...
  const handleLocationSelect = useCallback((lat: number, lng: number) => {
    ...
    if (shelters.length > 0) {
      const withDistance = shelters.map((shelter) => {
        ...
        const distance = calculateDistance(lat, lng, coords.coordinates[1], coords.coordinates[0]);
  ...
  const handleMapClick = (lat: number, lng: number) => {
    if (!isWithinBantul(lat, lng)) {
      toast.error("Maaf, wilayah yang Anda pilih di luar batas wilayah Kabupaten Bantul.");
      return;
    }
    ...
    if (!routeStart && !routeEnd) {
      ...
      setTimeout(async () => {
        try {
          const route = await roadApi.calculateRoute(routeStart.lat, routeStart.lng, lat, lng);
          setCalculatedRoute(route);
          toast.success(`Rute ditemukan! Jarak: ${(route.properties.totalDistance / 1000).toFixed(2)} km`);
        } catch (error) {
      ...
      setTimeout(async () => {
        try {
          const route = await roadApi.calculateRoute(lat, lng, routeEnd.lat, routeEnd.lng);
          setCalculatedRoute(route);
          toast.success(`Rute ditemukan! Jarak: ${(route.properties.totalDistance / 1000).toFixed(2)} km`);
        } catch (error) {
          console.error("Error calculating route:", error);
  ...
  const calculateRouteToShelter = async (shelterLat: number, shelterLng: number, shelterName: string) => {
    if (routingMode) {
      if (routeStart) {
        ...
        try {
          const route = await roadApi.calculateRoute(routeStart.lat, routeStart.lng, shelterLat, shelterLng);
          setCalculatedRoute(route);
          setRouteEnd({ lat: shelterLat, lng: shelterLng });
          toast.success(`Rute ke ${shelterName} ditemukan! Jarak: ${(route.properties.totalDistance / 1000).toFixed(2)} km`);
        } catch (error) {
          console.error("Error calculating route:", error);
          toast.error("Gagal menghitung rute. Pastikan titik awal terhubung dengan jalan.");
        } finally {
          setCalculatingRoute(false);
        }
      } else {
        setRouteEnd({ lat: shelterLat, lng: shelterLng });
        toast.info(`Tujuan di set ke ${shelterName}. Klik pada peta untuk memilih titik awal evakuasi Anda.`);
      }
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          const route = await roadApi.calculateRoute(userLat, userLng, shelterLat, shelterLng);
          setCalculatedRoute(route);
          ...
          console.error("Error calculating route:", error);
          toast.error("Gagal menghitung rute. Pastikan lokasi terhubung dengan jalan.");
        } finally {
          setCalculatingRoute(false);
        }
      },
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 min-h-screen space-y-6 max-w-[1600px] mx-auto">
      ...
      {isMapExpanded && (
        <div className="fixed inset-0 z-[90] bg-slate-900/40 dark:bg-zinc-950/60 backdrop-blur-sm transition-opacity" onClick={() => setIsMapExpanded(false)} />
      )}
      ...
      <div className={
        ...
        <div className="absolute top-4 right-4 z-[1000] flex flex-col gap-2">
           ...
           <Sheet open={sheetOpen} onOpenChange={setSheetOpen} modal={false}>
            ...
            <SheetContent side="right" className="w-[380px] p-0 bg-slate-50 dark:bg-zinc-950 border-l border-slate-200 dark:border-zinc-800 z-[1001]">
              ...
              <div className="overflow-y-auto h-[calc(100vh-5rem)] p-5 space-y-4">
                ...
                {selectedLocation && (
                  <div className="space-y-3">
                    <h2 className="text-sm font-bold text-slate-900 dark:text-zinc-100 border-b border-slate-200 dark:border-zinc-800 pb-2">Lokasi Terpilih</h2>
                ...
                {nearestShelters.length > 0 && (
                  <div className="space-y-3">
                    <h2 className="text-sm font-bold text-slate-900 dark:text-zinc-100 border-b border-slate-200 dark:border-zinc-800 pb-2">Shelter Terdekat</h2>
                    <div className="space-y-2">
                      {nearestShelters.map((shelter, i) => (
                        <Card 
                          ...
                          onClick={() => setExpandedShelterId(expandedShelterId === shelter.id ? null : shelter.id)}
                        >
                          <CardContent className="pt-4 pb-4 flex flex-col gap-2 text-sm">
                             <div className="flex justify-between items-start">
                               <div className="pr-2">
                                 <p className="font-semibold text-slate-800 dark:text-zinc-200">{shelter.name}</p>
                                 <p className="text-xs text-slate-500">{shelter.distance.toFixed(2)} km</p>
                               </div>
                             ...
                             {expandedShelterId === shelter.id && (
                               <div className="mt-2 pt-3 border-t border-slate-100 dark:border-zinc-800/60 animate-in fade-in slide-in-from-top-2 flex flex-col gap-2.5">
                                 <div className="grid grid-cols-2 gap-2">
                                    <div>
                                      <p className="text-[10px] uppercase text-slate-500 font-bold tracking-wider">Kondisi</p>
                                      <p className="text-xs font-medium text-slate-700 dark:text-zinc-300">{shelter.condition}</p>
                                    </div>
                                    <div>
                                      <p className="text-[10px] uppercase text-slate-500 font-bold tracking-wider">Kapasitas</p>
                                      <p className="text-xs font-medium text-slate-700 dark:text-zinc-300">{shelter.capacity} Orang</p>
                                    </div>
                                 </div>
                                 <div>
                                    <p className="text-[10px] uppercase text-slate-500 font-bold tracking-wider">Alamat</p>
                                    <p className="text-xs font-medium text-slate-700 dark:text-zinc-300 line-clamp-2" title={shelter.address || ""}>{shelter.address || "-"}</p>
                                 </div>
                                 <Button 
                                   size="sm" 
                                   ...
                                   onClick={(e) => {
                                     e.stopPropagation();
                                     const coords = shelter.geometry as { coordinates: [number, number] };
                                     calculateRouteToShelter(coords.coordinates[1], coords.coordinates[0], shelter.name);
                                   }}
                                 >
                ...
                {/* Hazard Zones */}
                <div className="space-y-3">
                   <h2 className="text-sm font-bold text-slate-900 dark:text-zinc-100 border-b border-slate-200 dark:border-zinc-800 pb-2">Zona Rawan</h2>
                   <Card className="bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 shadow-sm">
                      <CardContent className="pt-4 pb-4 space-y-2">
                        {hazardZones.slice(0, 4).map(zone => (
                           <div key={zone.id} className="flex justify-between items-center text-sm">
                             <span className="text-slate-700 dark:text-zinc-300 truncate">{zone.name}</span>
                             <Badge variant={zone.level === "CRITICAL" ? "destructive" : "secondary"} className="text-[10px]">{zone.level}</Badge>
                           </div>
                        ))}
                      </CardContent>
                   </Card>
           ...
           >
            {isMapExpanded ? <Minimize2 className="w-5 h-5 text-slate-700 dark:text-zinc-300 group-hover:scale-95 transition-transform" /> : <Maximize2 className="w-5 h-5 text-slate-700 dark:text-zinc-300 group-hover:scale-105 transition-transform" />}
           </Button>
        </div>
        ...
        <Button
          ...
          size="icon"
        >
          {gettingLocation ? <Loader2 className="h-5 w-5 text-white animate-spin" /> : <Navigation className="h-5 w-5 text-white" />}
        </Button>
        ...
        <Button
          ...
          className={`absolute bottom-8 right-4 z-[1000] h-12 w-12 shadow-lg border-0 rounded-full transition-colors flex items-center justify-center ${
            routingMode ? "bg-green-600 hover:bg-green-700" : "bg-slate-800 dark:bg-zinc-800 hover:bg-slate-900"
          }`}
          size="icon"
        ...
        {selectedEarthquake && (
          <Card className="absolute top-20 md:top-20 left-1/2 transform -translate-x-1/2 md:-translate-x-0 md:left-4 md:right-auto z-[1000] w-[320px] md:w-[360px] shadow-2xl bg-white dark:bg-zinc-950 border-slate-200 dark:border-zinc-800">
            <CardHeader className="pb-3 border-b border-slate-100 dark:border-zinc-800">
              <div className="flex items-center justify-between">
                <Badge className="bg-red-100 text-red-700 hover:bg-red-100 dark:bg-red-900/30 dark:text-red-400">Pusat Gempa Terpilih</Badge>
                <Button variant="ghost" size="icon" className="h-6 w-6" onClick={handleCloseEarthquakeDetail}>
                  <X className="h-4 w-4" />
                </Button>
              </div>
            </CardHeader>
            <CardContent className="pt-4 space-y-3">
              <h3 className="text-lg font-bold text-slate-900 dark:text-zinc-100 leading-tight">{selectedEarthquake.location}</h3>
              <div className="flex justify-between items-center text-sm p-2 bg-slate-50 dark:bg-zinc-900 rounded">
                 <span className="text-slate-600 dark:text-zinc-400 font-medium">Magnitudo</span>
                 <span className="font-bold text-red-600 dark:text-red-400">M {selectedEarthquake.magnitude}</span>
              </div>
              <div className="flex justify-between items-center text-sm p-2 bg-slate-50 dark:bg-zinc-900 rounded">
                 <span className="text-slate-600 dark:text-zinc-400 font-medium">Kedalaman</span>
                 <span className="font-bold text-slate-800 dark:text-zinc-200">{selectedEarthquake.depth} Km</span>
              </div>
              <p className="text-xs text-slate-500 pb-2">
                {new Date(selectedEarthquake.time).toLocaleDateString("id-ID", { day: "2-digit", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit" })} WIB
              </p>
            </CardContent>
          </Card>
        )}
        ...
        {calculatedRoute && (
          <Card className="absolute top-20 md:top-20 left-1/2 md:left-4 transform -translate-x-1/2 md:translate-x-0 z-[1000] w-[300px] shadow-xl bg-white dark:bg-zinc-950 border-slate-200 dark:border-zinc-800">
            <CardHeader className="pb-3 border-b border-slate-100 dark:border-zinc-800">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm text-slate-800 dark:text-zinc-200">Informasi Rute</CardTitle>
                <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => { setCalculatedRoute(null); setRouteStart(null); setRouteEnd(null); }}>
                  <X className="h-4 w-4" />
                </Button>
              </div>
            </CardHeader>
                ...
                return (
                  <>
                    <div className="flex items-center justify-between p-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-900/30">
                      <span className="text-xs font-semibold text-blue-800 dark:text-blue-300 uppercase tracking-wide">Jarak Jangkauan</span>
                      <span className="text-sm font-bold text-blue-600 dark:text-blue-400">{distKm.toFixed(2)} km</span>
                    </div>
                    
                    <div className="pt-1">
                      <span className="font-bold text-[10px] uppercase text-slate-400 tracking-wider mb-2 block">Estimasi Waktu Tempuh</span>
                      <div className="grid grid-cols-3 gap-2">
                        <div className="flex flex-col items-center justify-center bg-green-50 dark:bg-green-900/10 rounded-lg p-2 border border-green-100 dark:border-green-900/30">
                          ...
                          <span className="font-bold text-sm text-green-700 dark:text-green-500">{Math.ceil(walkTime)}<span className="text-[10px] font-normal">'</span></span>
                          <span className="text-[9px] text-green-600 dark:text-green-600/80 font-medium tracking-wide">Jalan</span>
                        </div>
                        <div className="flex flex-col items-center justify-center bg-orange-50 dark:bg-orange-900/10 rounded-lg p-2 border border-orange-100 dark:border-orange-900/30">
                          ...
                          <span className="font-bold text-sm text-orange-700 dark:text-orange-500">{Math.ceil(bikeTime)}<span className="text-[10px] font-normal">'</span></span>
                          <span className="text-[9px] text-orange-600 dark:text-orange-600/80 font-medium tracking-wide">Motor</span>
                        </div>
                        <div className="flex flex-col items-center justify-center bg-indigo-50 dark:bg-indigo-900/10 rounded-lg p-2 border border-indigo-100 dark:border-indigo-900/30">
                          ...
                          <span className="font-bold text-sm text-indigo-700 dark:text-indigo-500">{Math.ceil(carTime)}<span className="text-[10px] font-normal">'</span></span>
                          <span className="text-[9px] text-indigo-600 dark:text-indigo-600/80 font-medium tracking-wide">Mobil</span>
                        </div>
                      </div>
                    </div>
                  </>
          ...
          <Card className="absolute top-20 md:top-20 left-1/2 md:left-4 transform -translate-x-1/2 md:translate-x-0 z-[1000] w-[300px] shadow-xl bg-white dark:bg-zinc-950 border-slate-200 dark:border-zinc-800">
            <CardHeader className="pb-3 border-b border-slate-100 dark:border-zinc-800">
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => { setSelectedLocation(null); setNearestShelters([]); }}>
                  <X className="h-4 w-4" />
                </Button>
              </div>
            </CardHeader>
            <CardContent className="pt-4 space-y-2">
              ...
              {nearestShelters.length > 0 && (
                 <div className="text-xs text-slate-600 dark:text-zinc-400">
                    Ada <span className="font-medium text-slate-800 dark:text-zinc-200">{nearestShelters.length} shelter terdekat</span> dari titik. <br/>Buka profil Layer di kanan atas u/ info detail.
                 </div>
              )}
            </CardContent>
          </Card>
      ...
      <h2 className="text-xl font-bold tracking-tight text-slate-900 dark:text-zinc-50 pt-4">Dashboard Analitik</h2>
          ...
          <CardContent className="flex-1 min-h-[250px] pt-4 pr-6">
             <ResponsiveContainer width="100%" height={250} minWidth={0}>
              <LineChart data={chartData}>
                ...
                <XAxis dataKey="name" stroke={isDark ? "#71717a" : "#64748b"} fontSize={12} tickLine={false} axisLine={false} />
                <YAxis stroke={isDark ? "#71717a" : "#64748b"} fontSize={12} tickLine={false} axisLine={false} tickFormatter={(val) => `M${val}`} />
                <Tooltip 
                  contentStyle={{ backgroundColor: isDark ? "#18181b" : "#ffffff", borderColor: isDark ? "#27272a" : "#e2e8f0", borderRadius: '8px' }}
                  itemStyle={{ color: isDark ? "#e4e4e7" : "#0f172a" }}
                />
                <Line type="monotone" dataKey="magnitudes" stroke="#22c55e" strokeWidth={3} dot={{ r: 4, strokeWidth: 2 }} activeDot={{ r: 6, strokeWidth: 0 }} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card className="border border-slate-200 dark:border-zinc-800/50 bg-white dark:bg-zinc-950/80 shadow-sm flex flex-col pt-2">
          <CardHeader className="pb-3 border-b border-slate-100 dark:border-zinc-800/50">
            <div className="flex justify-between items-center">
              <CardTitle className="text-lg font-semibold text-slate-800 dark:text-zinc-200">Peringatan Dini</CardTitle>
              <Badge variant="outline" className="dark:text-zinc-400 dark:border-zinc-700 bg-slate-50 dark:bg-zinc-900">
          ...
          <CardContent className="p-0 overflow-auto max-h-[290px]">
            <div className="flex flex-col divide-y divide-slate-100 dark:divide-zinc-800/50">
              {earthquakes.length === 0 ? (
                <div className="p-4 flex items-center justify-center text-slate-500 h-[100px]">Tidak ada data.</div>
              ) : (
                earthquakes.slice(0, 5).map((eq, i) => (
                  <div key={eq.id || i} className="p-4 hover:bg-slate-50 dark:hover:bg-zinc-900/30 transition-colors cursor-pointer" onClick={() => handleEarthquakeClick(eq)}>
                    <div className="flex justify-between items-start mb-2 gap-2">
                       <span className="font-mono text-xs font-bold text-slate-900 dark:text-zinc-200">
                          {eq.location.slice(0, 35)}{eq.location.length > 35 ? '...' : ''}
                       </span>
                       <Badge className={
                         eq.magnitude >= 6 ? "bg-red-500/10 text-red-600 dark:text-red-400 hover:bg-red-500/20" :
                         eq.magnitude >= 5 ? "bg-orange-500/10 text-orange-600 dark:text-orange-400 hover:bg-orange-500/20" :
                         "bg-yellow-500/10 text-yellow-600 dark:text-yellow-400 hover:bg-yellow-500/20"
                       }>
                         {eq.magnitude >= 6 ? 'Bahaya' : eq.magnitude >= 5 ? 'Waspada' : 'Siaga'}
                       </Badge>
                    </div>
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 mt-2">
                      <div className="flex flex-col">
                        <span className="text-[10px] uppercase text-slate-500 font-semibold tracking-wider">Magnitudo</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-zinc-300">M {eq.magnitude}</span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[10px] uppercase text-slate-500 font-semibold tracking-wider">Waktu Waktu</span>
                        <span className="text-sm font-medium text-slate-700 dark:text-zinc-300">
                          {new Date(eq.time).toLocaleDateString('id-ID', { month: 'short', day: 'numeric'})}
                        </span>
                      </div>
                    </div>
                  </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pb-8">
        <Card className="border border-slate-200 dark:border-zinc-800/50 bg-white dark:bg-zinc-950/80 shadow-sm col-span-1 lg:col-span-2 pt-2">
           <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold text-slate-800 dark:text-zinc-200">Overview Status Wilayah</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="flex gap-4 text-xs font-medium text-slate-600 dark:text-zinc-400">
               <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-green-500" /> Aman</div>
               <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-yellow-400" /> Waspada</div>
               <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-orange-500" /> Siaga</div>
               <div className="flex items-center gap-1.5"><div className="w-2.5 h-2.5 rounded-full bg-red-500" /> Bahaya</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border border-slate-200 dark:border-zinc-800/50 bg-white dark:bg-zinc-950/80 shadow-sm flex items-center p-6 gap-6 relative overflow-hidden">
           <div className="flex items-center justify-center">
             <div className="text-5xl font-bold text-blue-600 dark:text-blue-400 drop-shadow-sm">+{shelters.length}</div>
           </div>
           <div className="flex flex-col relative z-10">
             <span className="text-lg font-bold text-slate-800 dark:text-zinc-100">Shelter Aktif</span>
        ...
        <Card className="border border-slate-200 dark:border-zinc-800/50 bg-white dark:bg-zinc-950/80 shadow-sm flex items-center p-6 gap-6 relative overflow-hidden">
           <div className="flex items-center justify-center">
             <div className="text-5xl font-bold text-amber-600 dark:text-amber-400 drop-shadow-sm">{wilayahStatus.status}</div>
           </div>
           <div className="flex flex-col relative z-10">
             <span className="text-lg font-bold text-slate-800 dark:text-zinc-100">Status Saat Ini</span>
             <span className="text-xs text-slate-500">Berdasarkan data rilis</span>
           </div>
        </Card>
      </div>
    </div>
