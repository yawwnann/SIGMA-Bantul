...
export default function AdminFacilitiesPage() {
  ...
  const [editingFacility, setEditingFacility] = useState<PublicFacility | null>(null);
  ...
  const handleEdit = (facility: PublicFacility) => {
    ...
    const coords = (facility.geometry as { coordinates?: [number, number] })?.coordinates;
    setFormData({
      name: facility.name,
      type: facility.type,
  ...
  return (
    <div className="py-6 w-full px-4 sm:px-6 md:px-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button onClick={openAddDialog} className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-900/20">
          ...
          <Table>
            <TableHeader className="bg-zinc-950/50">
              <TableRow className="border-b border-zinc-800 hover:bg-transparent">
                <TableHead className="font-semibold text-zinc-400">Nama Fasilitas</TableHead>
                <TableHead className="font-semibold text-zinc-400">Jenis Fasilitas</TableHead>
            ...
            <TableBody>
              {loading ? (
                ...
                facilities.map((facility) => (
                  <TableRow key={facility.id} className="border-b border-zinc-800/50 hover:bg-zinc-800/20 transition-colors">
                    <TableCell className="font-medium text-zinc-200">{facility.name}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-zinc-400">
                      <div className="flex items-center gap-1.5"><MapPin className="h-3 w-3 shrink-0"/> {facility.address || "Area Tidak Diketahui"}</div>
                    </TableCell>
                    <TableCell className="text-right space-x-2">
                      <Button variant="ghost" size="sm" onClick={() => handleEdit(facility)} className="text-blue-400 hover:text-blue-300 hover:bg-blue-500/10">
                        Edit
                      </Button>
                      <Button
                        variant="ghost"
        ...
        <DialogContent className="max-w-md bg-zinc-900 text-zinc-100 border-zinc-800">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              <Label htmlFor="name" className="text-zinc-400">Nama Fasilitas</Label>
              <Input
                id="name"
                value={formData.name}
            ...
            <div className="grid gap-2">
              <Label htmlFor="type" className="text-zinc-400">Jenis Fasilitas</Label>
              <Select
                value={formData.type}
                onValueChange={(value) => setFormData({ ...formData, type: value || "HOSPITAL" })}
              >
            ...
            <div className="grid gap-2">
              <Label htmlFor="address" className="text-zinc-400">Alamat Lengkap</Label>
              <Input
                id="address"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="grid gap-2">
                <Label htmlFor="lon" className="text-zinc-400">Longitude</Label>
                <Input
                  id="lon"
                  type="number"
                  step="any"
          ...
          <DialogFooter className="gap-2 sm:gap-0 mt-4">
            ...
            <Button onClick={handleSubmit} className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg shadow-emerald-900/20">
