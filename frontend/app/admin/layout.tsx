...
import { LayoutDashboard, Home, Building2, Map as MapIcon, LogOut, ShieldAlert, UserCircle } from "lucide-react";

const menuItems = [
  { href: "/admin/dashboard", label: "Dashboard", icon: LayoutDashboard },
  { href: "/admin/shelters", label: "Shelter Evakuasi", icon: Home },
  { href: "/admin/facilities", label: "Fasilitas Umum", icon: Building2 },
  { href: "/admin/routes", label: "Jalur Evakuasi", icon: MapIcon },
];
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    console.log("[AdminLayout] token in localStorage:", token ? "EXISTS" : "MISSING");
    ...
    <div className="dark min-h-screen w-full flex bg-zinc-950 text-zinc-50">
      ...
      <aside className="hidden md:flex w-64 bg-zinc-950 border-r border-zinc-800 flex-col fixed inset-y-0 left-0 z-50 shadow-xl">
        <div className="h-16 px-6 border-b border-zinc-800 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-zinc-50 tracking-tight leading-none">SIGMA Bantul</h1>
        ...
        <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
          <div className="text-[11px] font-semibold text-zinc-500 uppercase tracking-widest mb-4 px-2">Menu Utama</div>
          {menuItems.map((item) => {
        ...
        <div className="p-4 border-t border-zinc-800 bg-zinc-900/50">
          <div className="flex items-center gap-3 mb-4 px-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-zinc-500 font-medium leading-tight">Logged in as</p>
