...
export const shelterApi = {
  ...
  assignOfficer: async (shelterId: number, officerId: number) => {
    const response = await apiClient.put(`/shelters/${shelterId}/assign`, {
      officerId,
    });
    return response.data;
  },

  unassignOfficer: async (shelterId: number) => {
    const response = await apiClient.delete(`/shelters/${shelterId}/assign`);
    return response.data;
  },
};
