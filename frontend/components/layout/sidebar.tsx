...
export function Sidebar() {
  ...
  const { collapsed: contextCollapsed, toggleSidebar, mobileOpen, toggleMobile } = useSidebar();
  ...
  return (
    <>
      ...
      >
      <div className={`relative flex items-center border-b border-slate-200 dark:border-zinc-800 py-5 transition-all duration-300 ${collapsed ? "justify-center" : "px-6"}`}>
        ...
        >
          {collapsed ? <ChevronRight className="h-3 w-3 inline-block relative left-px" /> : <ChevronLeft className="h-3 w-3 inline-block relative right-px" />}
        </button>
      </div>
      ...
      <nav className={`flex-1 space-y-1 py-4 ${collapsed ? "px-2" : "px-3"}`}>
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <item.icon
                className={cn("shrink-0 h-5 w-5", isActive ? "text-white" : "")}
              />
      ...
      <div className={`border-t border-slate-200 dark:border-zinc-800 py-4 transition-all ${collapsed ? "px-2" : "px-3"}`}>
        {collapsed ? (
           <div className="flex justify-center" title="Status Darurat: Saat gempa terjadi, segera menuju shelter terdekat.">
          ...
          <div className="rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 p-4">
            ...
            <p className="text-xs text-slate-600 dark:text-red-200/70 mt-1">
              Saat gempa terjadi, segera menuju shelter terdekat dan hindari zona rawan.
            </p>
          </div>
        )}
      </div>
