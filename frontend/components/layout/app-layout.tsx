...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <main className={`flex-1 min-h-screen flex flex-col overflow-auto transition-all duration-300 ${collapsed ? "md:pl-[80px]" : "md:pl-64"}`}>
      {/* Mobile Top Bar */}
      <div className="md:hidden flex flex-none items-center justify-between p-4 border-b border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 sticky top-0 z-30">
        <div className="flex items-center gap-3">
          <button onClick={toggleMobile} className="p-1 rounded hover:bg-slate-100 dark:hover:bg-zinc-800">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-900 dark:text-zinc-50">SIGMA Bantul</span>
          </div>
        </div>
      </div>
